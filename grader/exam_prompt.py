...
def format_exam_grading_prompt(extracted_key: dict, free_response_rubric: dict = None,
    ...
    lines.extend([
        "",
        "=== COMMENT STYLE — match the instructor's voice ===",
        "The COMMENTS field must sound like a note from the teacher, not a bot. Mirror these rules:",
        "1. STRUCTURE: opener (one specific positive) -> 1-2 pattern-level error notes -> warm close. ONE short paragraph, 1-3 sentences total. Per-question detail belongs in QUESTION BREAKDOWN, not COMMENTS.",
        "2. POINT FORMAT: any deduction reference ends with the amount in parentheses, e.g. '(-5)', '(-2.5)', '(-1)'. NEVER write '-5 pts', '5 points off', or 'Deduction: 5'.",
        "3. VOICE: first-person, conversational, instructional. Kind first, corrective second. No hedges ('unfortunately', 'I'm afraid', 'please note that'). No academic-speak ('demonstrates insufficient depth').",
        "4. SIGNATURE PHRASING — use verbatim when the situation fits:",
        "   - Math wrong but method right: 'Math errors, followed your mistake through. (-2)' — cap at -2.",
        "   - Thin-but-present analysis (THIN_ANALYSIS): coach without deducting — 'Good start on your conclusion! To make it stronger, try to reference specific numbers or trends from your data...'",
        "   - First blank/near-blank submission: 'Please resubmit. This is a blank document. I will allow you to resubmit. If this happens again it will be graded as a 0.'",
        "5. OPENERS — rotate, never reuse the same one. Examples: 'Nice work on your dilution math.', 'Your DNA complement strands were clear.', 'Good handling of the pedigree question.' Prefer a SPECIFIC concrete detail (a number, a labeled element) over generic 'good job'.",
        "6. PERFECT-SCORE CLOSE: opener + one warm sign-off only. Examples: 'Great job! Keep up the excellent work.' / 'Excellent work, keep it up!' / 'Perfect score, well done!'",
        "7. SHIFT COMPENSATION: if a shift was applied, mention it plainly so the student knows (e.g., 'I shifted your answers down two to match the key.').",
        "8. AVOID AI TELLS: no em dashes used as drama beats, no 'it's not just X — it's Y', no 'let's dive in', no 'indeed', no 'essentially', no rule-of-three flourishes.",
        "9. FREE RESPONSE deductions go in FREE RESPONSE BREAKDOWN with the rubric->wrote->missing->level chain. COMMENTS stays warm and high-level.",
        "",
        ...
        "",
        "COMMENTS:",
        "[ONE short paragraph (1-3 sentences) in the instructor's voice — opener naming a specific strength, 1-2 pattern-level errors with point amounts in parens like '(-2)', any shift compensation noted. Per-question detail goes in QUESTION BREAKDOWN, not here.]",
        "",
